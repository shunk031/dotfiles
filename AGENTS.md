...
- Operation authorization: Treat requests such as "implement the plan" as authorization for repository-local edits, tests, and commits only. Do not infer authorization to push, create or update a pull request, merge, run `chezmoi apply`, modify runtime state, or delete or clean up files from a plan, handoff summary, or implementation request.
- Pull request scope: An explicit request to create or update a pull request authorizes the required push and that pull request operation, but never authorizes merging it. Merge only when the user explicitly asks to merge.
- Runtime and cleanup scope: Require explicit user authorization before running `chezmoi apply`, changing applied or runtime configuration, or deleting or cleaning up files. If the authorized operation is unclear, stop and ask before changing external or runtime state.
