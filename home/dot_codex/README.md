...
- The Codex-only source lives in [home/dot_config/codex/AGENTS.codex-only.md](../dot_config/codex/AGENTS.codex-only.md). It is source-only and is not applied directly under `~/.codex`; it exists because Codex reads a single home-level instruction file.
