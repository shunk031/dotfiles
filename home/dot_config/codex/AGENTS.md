...
- 定期確認: subagent の処理が 60〜90 秒を超えそうな場合は、親 agent から定期的に状態確認を行い、短い進捗をユーザへ返してください。
- 節目共有: 完了まで黙って待たず、節目ごとに現在地を共有してください。たとえば `gh_workflow_manager` では `worktree 準備`、`branch 作成`、`commit 作成`、`push 完了`、`PR 作成`、`CI 確認` などを区切りとして扱ってください。
- 停滞時: subagent からしばらく応答がない場合は、親 agent が割り込みや追跡で状態だけでも回収し、ユーザへ「返答待ち」ではなく「どこで止まっているか」を説明してください。
