...
- 原文にない短縮語やラベルを作っていないか。`主実行`、`primary`、`final` のように、読み手が意味を推測するしかない語は、平易な語に戻すか最初に定義する。
- 数字の単位と、何を数えているかが対応しているか。`行`、`件`、`クエリ`、`サンプル`、`レコード`、`トークン` を混同せず、`全24行` のような表現は対象を明記する。
- 略語、英語ラベル、処理段階名は最初の使用箇所で意味を示しているか。入力に定義がなければ、勝手に定義せず不足として指摘する。
