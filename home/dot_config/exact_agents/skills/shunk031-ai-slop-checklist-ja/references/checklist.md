...
- 原文にない短縮語やラベルを作っていないか。意味を推測させる語は、平易な語に戻すか最初に定義する。
- 数字の単位と、何を数えているかが対応しているか。単位の最初の使用箇所で数える対象を明記し、入力から対象が分からなければ推測せず不足として指摘する。
- 略語、英語ラベル、処理段階名を最初の使用箇所で定義し、その役割を示しているか。入力に定義がなければ、勝手に補わず不足として指摘する。
