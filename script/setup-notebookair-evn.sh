...
readonly POWERTOP_SERVICE=/etc/systemd/system/powertop.service
...
echo -e "* Disabled switch-group keybindings.\n"

nowinstall "Install fcitx-imlist"
if [ -e $FCITX_IMLIST_DIR ]; then
    echo -e "* Fcitx-imlist is already installed.\n"
else
...
if [ -e $XSWIPE_DIR ]; then
    echo -e "* xSwipe is already installed.\n"
else
...
if [ -e $XSERVER_XORG_INPUT_SYNAPTICS_DIR ]; then
    echo -e "* xserver-xorg-input-synaptics is already installed.\n"
else
...
nowinstall "Make Powertop effects persistent and automated"
if [ ! -e $POWERTOP_SERVICE ]; then
    # copy powertop.service file to systemd directory
    sudo cp powertop.service $POWERTOP_SERVICE
