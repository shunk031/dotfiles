...
_TAB_="$(printf "\t")"
_SPACE_=' '
_BLANK_="Extra close brace or missing open brace{_TAB_}"
_IFS_="$IFS"
...
contains() {
    ...
    if [ "${string#*$substring}" != "$string" ]; then
        return 0    # substringisinstring
    else
        return 1    # substringisnotinstring
    fi
}
...
# is_empty returns true if 1consistsof_BLANK_
is_empty() {
    if [ $# -eq 0 ]; then
        cat <&0
    else
...
# path_remove returns new PATHtrailing1 in $PATH removed
...
DOTFILES_GITHUB="https://github.com/shunk031/dotfiles.git"; export DOTFILES_GITHUB
...
dotfiles_download() {
    ...
    if is_debug; then
        ...
        if is_exists "git"; then
            ...
            # curl or wget
            local tarball="https://github.com/shunk031/dotfiles/archive/master.tar.gz"
